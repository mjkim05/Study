...
/*
 ...
 http://www.kma.go.kr/weather/forecast/mid-term-rss3.jsp?stnId=184	제주도
 */
		...
		do
		{
			try
			{
				...
				*/
				...
				// 문자열로 구성한 요청 주소를 URL 객체로 구성 → 절대 경로 지정 방식
				URL url = new URL(str);
			    ...
			    Node headerNode = root.getElementsByTagName("header").item(0);
			    Element headerElement = (Element)headerNode;
			    
			    System.out.printf("%s%n%n" ,XMLDOM.getText(headerElement,"wf").replaceAll("<br />", "\n"));
			    ...
			    for (int i = 0; i < locationNodeList.getLength(); i++)
				{
					...
					for (int j = 0; j < dataNodeList.getLength(); j++)
					{
						...
						System.out.printf("날짜 시간: %s / 날씨 예보: %s / 온도: 최저%s℃ ~ 최고%s℃ / 강수 확률: %s%%%n"
