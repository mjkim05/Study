...
{
	// ■ 전체 페이지 수를 구하는 메소드
	// numPerPage : 한페이지에 표시할 데이터(게시물)의 수
	...
	// totalPage : 전체 페이지 수
	// listUrl : 링크를 설정할 url ex) <a href=...>23</a>
	public String pageIndexList(int currentPage, int totalPage, String listUrl)
	{
		...
		//    1페이지도 못채우는 경우는 페이징 처리를 할 필요가 없다.
		if (currentPage==0)
			return "";
		...
		if (listUrl.indexOf("?") != -1) // 링크를 설정할 URL 에 ? 가 들어있다면...
		{
		...
		//    하지만, 현재 페이지가 11페이지라면 (currentPege=11)
		//    『11/10 = 1』 이며... 여기에 *10 (10을 곱하면) 10이다.
		//    그러면... curremtPageSetup 은 10이 되는 것이다.
		...
		// -- 만약 위 처리에서.. (라인 85)
		//    현재 페이지가 20 페이지 였다면.. (currentPage=20)
		//    20/10 = 2 이며 여기에 『*10』 (10을 곱해서) 20이 되는데
		//    이와 같은 상황이라면.. 다시 10을 빼서 10으로 만들어주기 위한 구문
		//    왜냐하면 currentPage 가 1이어야 11 ~ 20 까지 보여주기 때문!
		...
		// -- listUrl 은 (라인 72~79) 이미 전처리가 끝난 상황이기 때문에
		//    『....?』 상태 또는 『..?....&』 인 상태이다.
		//    이로 인해 결과는 
		...
		// Prev(이전으로)
		n = currentPage - numPerBlock; //(15 - 10) → 5페이지보여줌
		//-- n: 해당 페이지만큼 앞으로(또는 뒤로)가기위한 변수
		if ((totalPage>numPerBlock) && (currentPageSetup>0))
		{
		...
		//   이때, 현재 페이지가(currentPage) 가 11이상일 경우
		//   『Prev』 를 붙이기 위한 구문
		//-- 『Prev』 를 클릭할 경우
		//    n 변수 페이지로 이동하는데
		...
		// Next (다음으로)
		n = currentPage + numPerBlock;  //현재 28 페이지일 경우 38페이지로 이동할 수 있도록
		//		35          20 (21~30)         10
