...
SELECT NAME ,IBSADATE , FN_WORKYEAR2(IBSADATE)
FROM TBL_INSA;
...
-- 지금 작성하는 테이블은 바람직하지 않은 구조 일 수 있기때문에 참고하지 않기!! ㄴㄴ 
-- 프로시저 실습을 위해 만드는 것뿐이다!
...
(  ID       VARCHAR2(10)
 ...
 , TEL      VARCHAR2(30)
 , ADDR     VARCHAR2(100)
 , CONSTRAINT STUENTS_ID_PK PRIMARY KEY(ID)
);
--==>> Table TBL_STUDENTS이(가) 생성되었습니다.

-- 나중에 프로젝트 할 때 회원테이블이 있는데 로그인만 수행하는 아래와 같은 테이블 만들지 않기!!! 인덱스 사용
...
-- 이와 같은 구문 한줄로 양쪽 테이블 모두 제대로 데이터를 입력 할 수 있다.

--※ 프로시저 생성하는 구문은 
-- 『20231103_01_scott(plsql).sql』에서~
...
-- 이미 생성된 테이블에 있는 컬럼을 쿼리문을 통해 충분히 계산 할 수 있는 것들은 절대 절대 절대로 컬럼을 통해 데이터를 생성하지 말자!!!!!!!!!!!!
-- (총점→TOT, 평균→  AVG, 등급→GRADE) ← 이것도 마찬가지임!! 

ALTER TABLE TBL_SUNGJUK
ADD(TOT NUMBER(3), AVG NUMBER(4,1), GRAED CHAR);
--==>> Table TBL_SUNGJUK이(가) 변경되었습니다

--※ 여기에서 추가한 컬럼에 대한 항목은
--   프로시저 실습을 쉽게 진행 할 수 있도록 추가한 항목일뿐
--   실제 테이블 구조에 적합하지도 바람직하지도 않은 내용이다~~ CHECK~~~!!!!

--==>> 기존 테이블의 데이터를 기반으로 쿼리문을 활용하여 얻을 수 있는 데이터는
-- 절대 테이블에 추가로 구성(테이블내에 컬럼화)하지 않는다.
...
EXEC PRC_SUNGJUK_INSERT(2,'김민지',100,100,100);

EXEC PRC_SUNGJUK_INSERT(3,'최혜인',100,99,98);

EXEC PRC_SUNGJUK_INSERT(4,'이주형',55,40,80);

EXEC PRC_SUNGJUK_INSERT(5,'노은하',70,80,70);

EXEC PRC_SUNGJUK_INSERT(6,'채다선',70,50,60);
...
--○ 프로시저 생성 후 실행(호출)
EXEC PRC_SUNGJUK_UPDATE(2,70,70,70);
--==>> PL/SQL 프로시저가 성공적으로 완료되었습니다.
