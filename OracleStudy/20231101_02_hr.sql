...
--   (또한, 쿼리문에 대한 변경 결과를 확인 후 ROLLBACK 을 수행한다.)
...
-- IT부서 직원들의 FIRST_NAME , LAST_NAME, SALARY, DEPARTMENT_ID
...
-------- 스스로 해봤던 것 굳이 JOIN 안써두 됨 안쓰는게 더 좋음! 메모리 낭비------                         
UPDATE EMPLOYEES 
...
--   (또한, 변경에 대한 결과 확인 후 ROLLBACK 진행한다.)
...
SELECT CASE WHEN DEPARTMENT_ID = (SELECT DEPARTMENT_ID
            ...
            WHEN DEPARTMENT_ID = (SELECT DEPARTMENT_ID
                                  FROM DEPARTMENTS
                                  WHERE DEPARTMENT_NAME ='Accounting') THEN SALARY * 1.20 
       ELSE SALARY
       END "급여"
FROM EMPLOYEES;

-- 인상률
SELECT CASE WHEN DEPARTMENT_ID = 100 THEN SALARY  * 1.1 
            WHEN DEPARTMENT_ID = 90 THEN SALARY * 1.15
            WHEN DEPARTMENT_ID = 110 THEN SALARY * 1.20 
       ELSE -1 
       END "급여"
FROM EMPLOYEES
WHERE DEPARTMENT_ID IN (SELECT DEPARTMENT_ID
...
--> SELECT 먼저 하고 그 뒤에 DELETE로 바꿔서 사용하자~~!!     
