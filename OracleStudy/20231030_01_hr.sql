...
가정) 서울 여의도 LG(본사)라는 회사에 근무하는 거래처 직원 명단이
      ...
      사옥을 이전하게 되었다.
      회사 주소는 『경기분당』으로 바뀌고, 회사전화는 『031-111-2222』로 바뀌게 되었다.
...
-- 거래처회사명, 거래처직원명
SELECT 거래처회사명, 거래처직원명      | SELECT 거래처직원명, 직급
...
------------------------------------------------------------------------ 
  ...
  PKY5834(박가영)  P-SWKK(새우깡)     2023-10-30 09:07:04      12
                                  ...
                                  :
------------------------------------------------------------------------                                 
PK 고객ID, 제품코드가 될 수 없음, 왜냐 중복이 될 수 없기 때문에 여러개의 주문이 들어올 수 없음
주문 테이블에는 PK라고 할 수 있는게 없음
*/
...
-------------------------------------------------------------------------
  ...
  O3188   오라클중급  22     테슬라       T301    전산실습관 3층 40석 규모
  O3189   오라클중급  10     장영실       T502    전자공학관 5층 60석 규모 
...
--  『1(일) : many(다)』 관계로 깨트리는 과정이 바로 제 4 정규화 수행 과정이다.
...
-- 부서번호 부서명 주소     사원번호 사원명 직급 급여 입사일 부서번호     부서명
-----------------------    ---------------------------------------- + --------
-- 500000개 레코드(행)          1,000,000개 레코드(행)
-----------------------    ---------------------------------------- + --------
...
-- 그냥 내가 적은 필기 
-- 부모 테이블과 자식 테이블의 레코드 개수가 많이 차이나면 역정규화를 하지 않는 것이 좋고
-- 별 차이가 안나면 역정규화를 하는 것이 좋다.
...
2. 개체 무결성 (Entity Integrity)
   ...
   유일성을 보장하기 위한 제약조건이다.
   ex) EMP 테이블에 조건이 모두 같은 SMITH 2명
   
3. 참조 무결성(Relational Integrity)
   참조 무결성은 릴레이션 간의 데이터 일관성을
   보장하기 위한 제약조건이다.
   ex) 박가영, 부서번호 60인데 DEPT 테이블에 부서번호 60은 존재하지 않음
...
-- 3. 형식 및 구조
-- ① 컬럼 레벨의 형식
-- 컬럼명 데이터 타입 [CONSTRAINT CONSTRAINT명] PRIMARY KEY[(컬럼명, ...)]          []안에 있는 것은 생략 가능한 애들

-- ② 테이블 레벨의 형식 (컬럼 레벨 형식으로 만들라는 말이 없다면 이걸로 작성해봅시다~ 왜냐면 컬럼 레벨은 짧아서 컬럼 레벨만 하다보면 테이블 레벨로 작성하라고 하면 잘 못하는 경우가 많음)
-- 컬럼명 데이터타입,
-- 컬럼명 데이터타입,
