...
--==>>
/*
BIN$7Vd1bCS6Ryi0mcX9MZq4CQ==$0	TABLE	
BIN$CpajJDrHREyEYCdPggEDCA==$0	TABLE	
BONUS	                        TABLE	
DEPT	                        TABLE	
EMP	                            TABLE	
EMP2	                        TABLE	
...
EMP2	        USERS			VALID	10		1	255	65536	1048576	1	2147483645				YES	N	14	5	0	0	0	38	0	0	         1	         1	    N	ENABLED	14	2023-10-20	NO		N	N	NO	DEFAULT	DEFAULT	DEFAULT	DISABLED	YES	NO		DISABLED	YES		DISABLED	DISABLED		NO	NO	YES	DEFAULT
...
TBL_JUMUNBACKUP	USERS			VALID	10		1	255	65536	1048576	1	2147483645				YES	N	20	4	0	0	0	24	0	0	         1	         1	    N	ENABLED	20	2023-10-27	NO		N	N	NO	DEFAULT	DEFAULT	DEFAULT	DISABLED	YES	NO		DISABLED	YES		DISABLED	DISABLED		NO	NO	YES	DEFAULT
...
FROM TBL_INSA
WHERE CITY = '서울' AND (BASICPAY / 10000) >= 150;
--==>>
/*
...
SELECT *
FROM TBL_INSA
WHERE CITY = '인천' AND (BASICPAY / 10000) BETWEEN 100 AND 200; 
--==>>
/*
1014	황진이	810707-2574812	2002-02-15	인천	010-3214-5467	개발부	사원	1100000	130000
1035	정영희	831010-2153252	2002-05-16	인천		            개발부	사원	1050000	140000
...
FROM TBL_INSA
WHERE SUBSTR(IBSADATE,1,3) = 200;

--20. 2000년 10월에 입사한 사람 조회. (이름, 출신도, 입사일).
SELECT NAME "이름" ,CITY "출신도",IBSADATE "입사일" 
FROM TBL_INSA
WHERE IBSADATE >= TO_DATE('2000-10','YYYY-MM') AND IBSADATE <= TO_DATE('2000-11' , 'YYYY-MM');
--==>>
/*
김정훈	전북	2000-10-01
우재옥	서울	2000-10-01
...
SELECT *
FROM TBL_INSA
WHERE 20 = TRUNC(EXTRACT(YEAR FROM SYSDATE) - (SUBSTR(SSN,1,2)+1899),-1);
--==>> 20대인 사원이 없기 때문에 조회결과 없음
...
SELECT *    
FROM TBL_INSA
WHERE TO_CHAR(SSN) LIKE '___5__________';
--==>>
/*
1006	이기자	780505-2978541	2002-02-11	인천	010-3214-5357	개발부	과장	2265000	150000
1007	장인철	780506-1625148	1998-03-16	제주	011-2345-2525	개발부	대리	1250000	150000
...
FROM TBL_INSA
WHERE SUBSTR(SSN,8,1) IN ('1','3')
ORDER BY EXTRACT(YEAR FROM SYSDATE) - (SUBSTR(SSN,1,2)+1899);
...
--  없으면 '전화번호없음'으로 조회. 
SELECT NVL(REPLACE(TEL,'-',''),'전화번호없음') "번호"
FROM TBL_INSA; 
...
--          커미션없는사원    XXX
--          모든사원          XXX
SELECT NVL(T.커미션,'모든사원') , COUNT(*)
FROM 
(
    SELECT CASE WHEN COMMISSION_PCT  IS NULL THEN '커미션없는사원'
                WHEN COMMISSION_PCT  IS NOT NULL THEN '커미션받는사원'
                ELSE '오류' 
                END "커미션"
    FROM EMPLOYEES
)T
...
FROM
(
    SELECT CASE WHEN (BASICPAY + SUDANG) < 1000000  THEN '100만원미만'
                WHEN (BASICPAY + SUDANG)  BETWEEN 1000000 AND 2000000 THEN '100만원~200만원'
...
SELECT SUBSTR(SSN,1,2) "년도" , COUNT(*) "직원수"
FROM TBL_INSA
...
SELECT CASE WHEN SUBSTR(SSN,8,1) IN ('1','3') THEN '남자'
            ...
            WHEN SUBSTR(SSN,8,1) IS NULL      THEN '전체'
            ELSE '성별 알 수 없음'
            END "성별", COUNT(*) "인원수"
FROM TBL_INSA 
...
SELECT BUSEO , DECODE(BUSEO,'개발부',COUNT(*),'영업부',COUNT(*),'총무부',COUNT(*)) "인원수"
FROM TBL_INSA
...
--39. 서울 사람의 남자 인원수 조회.
SELECT DECODE(SUBSTR(SSN,8,1), '1','남자','3','남자') "성별" , COUNT(*) 인원수 
FROM TBL_INSA 
...
SELECT DECODE(SUBSTR(SSN,8,1),'1','남자','3','남자','2','여자','4','여자') "성별", COUNT(*) "인원수"
FROM TBL_INSA
WHERE BUSEO = '영업부'
GROUP BY SUBSTR(SSN,8,1);
--==>>
...
SELECT IBSADATE "입사년도"  
      ...
      ,COUNT(*)
FROM TBL_INSA
GROUP BY IBSADATE , SUBSTR(SSN,8,1);
--==>>
/*
2003-10-10	남자	1
1998-06-07	여자	2
...
(    
    SELECT BUSEO "부서"
          ...
          ,COUNT(*) "인원수"
    FROM TBL_INSA
    GROUP BY BUSEO, IBSADATE , SUBSTR(SSN,8,1)
)T
...
SELECT NAME "이름" , BASICPAY "기본급"    
FROM TBL_INSA
WHERE CITY = '서울' AND (BASICPAY /10000) >= 200;
--==>>
/*
홍길동	2610000
박문수	2300000
...
SELECT NAME "이름" , SUBSTR(SSN,1,2)+1900 || '-' || SUBSTR(SSN,3,2) || '-' || SUBSTR(SSN,5,2) "주민번호"
           , TO_CHAR(BASICPAY,'L999,999,999') "기본급" , SUDANG "수당"
FROM TBL_INSA;

    
SELECT NAME "이름" , SSN "주민번호" , BASICPAY "기본급" , "수당"
FROM TBL_INSA;
...
SELECT BUSEO "부서", JIKWI "직위" , COUNT(*) "인원수" , SUM(BASICPAY+SUDANG) "급여합" , TRUNC(AVG(BASICPAY+SUDANG)) "급여평균"
FROM TBL_INSA
GROUP BY BUSEO,JIKWI;
--==>>
/*
...
--70. 입사년도별 인원수 조회.
SELECT EXTRACT(YEAR FROM IBSADATE) "입사년도" , COUNT(*) "인원별"
FROM TBL_INSA
...
SELECT SUM(DECODE(TO_CHAR(IBSADATE ,'YYYY'),NULL,COUNT(*))) "전체"   
      ,SUM(DECODE(TO_CHAR(IBSADATE ,'YYYY'),'2000',COUNT(*))) "2000" 
      ,SUM(DECODE(TO_CHAR(IBSADATE ,'YYYY'),'1999',COUNT(*))) "1999"
      ,SUM(DECODE(TO_CHAR(IBSADATE ,'YYYY'),'1998',COUNT(*))) "1998"
FROM TBL_INSA
GROUP BY  ROLLUP(TO_CHAR(IBSADATE ,'YYYY'));     
--==>> 
/*
        전체       2000       1999       1998
---------- ---------- ---------- ----------
...
SELECT  SUM(DECODE(CITY,NULL,COUNT(*))) "전체"   ,SUM(DECODE(CITY,'서울',COUNT(*))) "서울" ,SUM(DECODE(CITY,'인천',COUNT(*))) "인천"
       ,SUM(DECODE(CITY,'경기',COUNT(*))) "경기" ,SUM(DECODE(CITY,'부산',COUNT(*))) "부산" ,SUM(DECODE(CITY,'충남',COUNT(*))) "충남" 
       ,SUM(DECODE(CITY,'강원',COUNT(*))) "강원" ,SUM(DECODE(CITY,'전북',COUNT(*))) "전북" ,SUM(DECODE(CITY,'제주',COUNT(*))) "제주" 
       ,SUM(DECODE(CITY,'경북',COUNT(*))) "경북" ,SUM(DECODE(CITY,'전남',COUNT(*))) "전남" ,SUM(DECODE(CITY,'경남',COUNT(*))) "경남" 
FROM TBL_INSA
GROUP BY  ROLLUP(CITY);
--==>>
/*
전체, 서울, 인천, 경기, 부산, 충남, 강원, 전북, 제주, 경북, 전남, 경남
60	    20	  9	   13	  3	    1	 2    	5	  2	   1	 3	    1 
*/
SELECT CITY
FROM TBL_INSA
GROUP BY CITY;
...
SELECT T.NAME "이름" , T.BUSEO "부서" , BASICPAY "기본급"
FROM 
(
...
*/
--85. 남, 여별 기본급 순위 조회.
SELECT T.BASICPAY , T.성별 , RANK() OVER(PARTITION BY T.성별 ORDER BY T.BASICPAY DESC) "순위"
FROM 
(
    SELECT BASICPAY , DECODE(SUBSTR(SSN,8,1),'1','남자','2','여자','3','남자','4','여자') "성별"
    FROM TBL_INSA
)T;
--==>>
/*
...
FROM 
(
    SELECT NAME, BASICPAY+SUDANG "수당", CITY "지역"
...
(
  ...
  , RANK() OVER(ORDER BY TRUNC(AVG(BASICPAY + SUDANG)) DESC) "급여순위"
    FROM TBL_INSA
    GROUP BY CITY
)T
WHERE T.급여순위 = 1;
...
SELECT T2.BUSEO "부서" , T2.성별 , T2.인원수
FROM 
(
    SELECT T1.* , RANK() OVER(ORDER BY T1.인원수 DESC) "순위"
    FROM 
    (
        SELECT BUSEO , DECODE(SUBSTR(SSN,8,1),'1','남자','2','여자','3','남자','4','여자') "성별"
              ,COUNT(*) "인원수" 
        FROM TBL_INSA 
        GROUP BY BUSEO, SUBSTR(SSN,8,1)
    ) T1
)T2
WHERE T2.순위 = 1 AND T2.성별 = '여자';
--==>>
/*
영업부	여자	8
개발부	여자	8
...
--    실수령액: 총급여-세금
SELECT A.* , (A.총급여 - 세금) "실수령액"
FROM 
(
    SELECT T.* 
          ,CASE WHEN T.총급여 >= 2500000 THEN T.총급여 * 0.2 
                WHEN T.총급여 >= 2000000 THEN T.총급여 * 0.1
                WHEN T.총급여 <  2000000 THEN  0
           ELSE 0
           END 세금
    FROM 
    (
...
FROM TBL_INSA
WHERE (BASICPAY + SUDANG) IN ( SELECT MAX(BASICPAY+SUDANG)
                               FROM TBL_INSA);
--==>>
/*
홍길동	2810000
